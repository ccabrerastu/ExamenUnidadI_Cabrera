...
import { Button, Form, Input, Popconfirm, Table, Modal, Layout, Menu, Typography, message, Select } from 'antd';
...
// Mapeo de tratamientos según el tipo de activo
const tratamientoPorTipo = {
  'Base de Datos': [
  ...
  'Servicio Web': [
    'Autenticación robusta y validación de inputs',
    'Monitoreo de APIs',
  ...
  'Aplicación': [
    ...
    'Cifrado de datos sensibles',
    'Capacitación de usuarios sobre seguridad'
  ],
  'Infraestructura': [
...
const App = () => {
  ...
  setTimeout(() => {
    const newDataSource = dataSource.map(item => {
      const posiblesTratamientos = tratamientoPorTipo[item.tipo] || ['Revisión general de seguridad'];
      return {
        ...item,
        tratamiento: posiblesTratamientos[Math.floor(Math.random() * posiblesTratamientos.length)]
      };
    });
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      >
        <Form layout="vertical">
          {/* Input de nombre de activo */}
          <Form.Item 
            label="Activo" 
            name="activo"   // <-- agregar name
          ...
          {/* Select para tipo de activo */}
          <Form.Item 
            label="Tipo de activo" 
            name="tipo"   // <-- agregar name
            rules={[{ required: true, message: 'Por favor selecciona un tipo de activo' }]}
          >
            <Select
              value={newData.tipo}
              onChange={(value) => setNewData({ ...newData, tipo: value })}
              placeholder="Selecciona el tipo"
            >
              <Select.Option value="Base de Datos">Base de Datos</Select.Option>
              <Select.Option value="Servicio Web">Servicio Web</Select.Option>
              <Select.Option value="Aplicación">Aplicación</Select.Option>
              ...
              <Select.Option value="Almacenamiento">Almacenamiento</Select.Option>
